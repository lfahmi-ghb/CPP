...
/*
int Menu::run() const {
	char choice = 0; //initialize
	bool value = display();
	int numberOfMenuItems = 0;

	for (int i = 0; i < MAX_NO_OF_ITEMS; i++)
	{
		if (MenuItems[i].m_item != nullptr) {
			++numberOfMenuItems;
		}
	}
	...
	do
	{
		...
		else if ((int)choice  > numberOfMenuItems || (int)choice <= 0)\\????????????
		{
			cout << "Invalid selection, try again: ";
...
int Menu::run() const {
	...
	display();
	while (notValid && numberOfMenuItems)
	{
		...
		else if ((int)choice-48   > numberOfMenuItems || (int)choice -48 <= 0)
		{
			cout << "Invalid selection, try again: ";
		}
		else
